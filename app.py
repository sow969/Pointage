        ...
        for (x, y, w, h) in features:
            ...
            if confidence > 70 and not justscanned:
                ...
                if int(cnt) == 30:
                    ...
                    mycursor.execute(
                        "insert into accs_hist (accs_date, accs_prsn, accs_added) values('"+str(date.today())+"', '" + str(pnbr) + "', '"+str(datetime.now())+"')")
                    mydb.commit()
...
def addprsn():
    ...
    return render_template('add.html', newnbr=int(nbr))
...
def addprsn_submit():
    ...
    prsadded = str(datetime.now())

    mycursor.execute("""INSERT INTO `prs_mstr` (`prs_nbr`, `prs_name`, `prs_skill`, `prs_added`) VALUES
                    ('{}', '{}', '{}', '{}')""".format(prsnbr, prsname, prsskill, prsadded))
    mydb.commit()
...
def loadData():
    ...
    mycursor.execute("select a.accs_id, a.accs_prsn, b.prs_name, b.prs_skill, date_format(a.accs_added, '%H:%i:%s') "
                     ...
                     " order by 1 desc")
    data = mycursor.fetchall()
    print("////////////////test",jsonify(reponse=data))
